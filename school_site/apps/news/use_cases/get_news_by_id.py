...
class GetNewsUseCaseProtocol(UseCaseProtocol[NewsReadSchema]):
    async def __call__(self, news_id: UUID) -> NewsReadSchema:
        ...
...
class GetNewsUseCase(GetNewsUseCaseProtocol):
    ...
    async def __call__(self: Self, news_id: UUID) -> NewsReadSchema:
